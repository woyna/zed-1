...
blood_test$gender <- as.factor(blood_test$gender)
blood_test$outcome <- as.factor(blood_test$outcome)
```
...
Poniżej zaprezentowano krótkie podsumowanie dotyczące analizowanych danych.

```{r gender_summary, results='markup'}
gender_age_survived_summary <- blood_test_grouped %>% summarize(age = first(age), gender=first(gender), outcome=first(outcome)) %>% mutate(age_group = factor(1*(age > 35) + 1*(age > 55), labels = c('18-35', '36-55', '56+'))) %>% mutate(gender = if_else(condition = gender == 1, 'female', 'male')) %>% mutate(survived = if_else(condition = outcome == 0, 'yes', 'no')) %>% group_by(gender, age_group, survived) %>% count(name = 'count')

summary_plot <- ggplot(gender_age_survived_summary, aes(x = gender, y = count, fill = survived)) + geom_col(width = 0.4) + facet_grid(. ~ age_group)
