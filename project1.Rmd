...
Raport ten przedstawia analizę wyników badań krwi pacjentów z szpitala Tongji w Wuhan chorych na COVID-19. W toku tej analizy odkryto silną korelację pomiędzy wiekiem i płcią pacjenta, a przeżywalnością choroby. Korelacja ta implikuje, że mężczyźni oraz młodzi ludzie mają większą szansę na przeżycie choroby. Kolejnym istotnym czynnikiem okazała się zawartość neutrofili we krwi. Zarówno liczba neutrofili, jak i procentowa zawartość w stosunku do wszystkich leukocytów, mogą służyć do predykcji stanu pacjenta. Podwyższony poziom neutrofili oznaczał mniejszą szansę na przeżycie. Następnym wskaźnikiem, który wpływa na przeżywalność, okazało się stężenie dehydrogenazy mleczanowej (LDH). Poziom LDH ponad normę zauważony był u większości pacjentów, którzy nie przeżyli zakażenia wirusem. W kolejnym kroku analizy zbadano zmienność wskaźników w czasie. W przypadku stężenia fosfotazy alkaicznej, stężenia bilirubiny bezpośredniej oraz hs-CRP zauważono średnio wyższy poziom oraz trend rosnący u pacjentów, którzy nie przeżyli. Z kolei poziom eGFR był średnio wyższy u pacjentów, którzy przeżyli zachorowanie na COVID-19. Wymienione wyżej atrybuty wykorzystane zostały do budowy klasyfikatora, określającego czy dany pacjent przeżyje zakażenie wirusem. Algorytmem zastosowanym w klasyfikatorze był algorytm C5.0 wsparty macierzą kosztów, zmniejszającą szansę na błędne sklasyfikowanie pacjenta chorego jako zdrowego. Dla zbioru testowego udało się osiągnąć precyzję na poziomie 93% i czułość na poziomie 100%.
...
```{r classification}
blood_test_grouped_final <- blood_test_grouped %>%
                  select(gender, age, neutrophils..., neutrophils.count, Lactate.dehydrogenase,
                         Alkaline.phosphatase, Direct.bilirubin, eGFR, High.sensitivity.C.reactive.protein, outcome)%>% 
                  mutate(gender = if_else(condition = gender == 1, 'female', 'male')) %>% 
                  mutate(outcome = if_else(condition = outcome == 0, 'tak', 'nie')) %>%
                  rename(survived = outcome)

#omit rows with NA value in any column
blood_test_grouped_final <- na.omit(blood_test_grouped_final)

#convert survived and gender attributes to binominal data type
blood_test_grouped_final$survived = factor(blood_test_grouped_final$survived)
blood_test_grouped_final$gender = factor(blood_test_grouped_final$gender)
...
inTraining <- 
  createDataPartition(
    y = blood_test_grouped_final$survived,
    p = .75,
    list = FALSE)

training <- blood_test_grouped_final[inTraining,]
testing <- blood_test_grouped_final[-inTraining,]

#create cost matrix
costMatrix <- matrix(c(0,1,15,0), ncol = 2)

colnames(costMatrix) <- levels(training$survived)
rownames(costMatrix) <- levels(training$survived)

#use 5x2 cross-validation
ctrl <- trainControl(
  method = "repeatedcv",
  number = 2,
  repeats = 5)

#use C5.0 model with cost matrix
set.seed(10)
fit <- train(survived ~ .,
             data = training,
             method = 'C5.0',
...
confMatrix <- confusionMatrix(data = classes, testing$survived)
```

Do utworzenia klasyfikatora próbującego przewidzieć, czy dany pacjent przeżyje, wybrano 9 atrybutów opisanych w powyższym raporcie. Te atrybuty to:
...
Danymi wykorzystanymi do wytrenowania i przetestowania klasyfikatora były średnie wartości wybranych atrybutów z badania krwi pacjentów. Odrzucono pacjentów, którzy nie mieli wyniku badania choć jednego z wytypowanych atrybutów. Zbiór ten podzielono następnie na dane uczące i testowe w stosunku 3:1.

Zbiór uczący:

```{r trainingSummary, results='markup'}
trainingSummary <- training %>% group_by(survived) %>%  count(name = 'liczebność') %>% rename(przeżył = survived)

trainingSummary %>% kbl() %>% kable_classic_2(full_width = F, position = "left") %>% row_spec(0, bold = T)
```

Zbiór testowy:

```{r testingSummary, results='markup'}
testingSummary <- testing %>% group_by(survived) %>%  count(name = 'liczebność') %>% rename(przeżył = survived)

testingSummary %>% kbl() %>% kable_classic_2(full_width = F, position = "left") %>% row_spec(0, bold = T)
```

Wykorzystanym schematem uczenia była powtórzona ocena krzyżowa (5x2). 

Na klasę pozytywną została wybrana została wartość "nie" (pacjent nie przeżył). W modelu klasyfikacyjnym wykorzystanym algorytmem został algorytm C5.0 (algorytm oparty na regułach). Dodatkowo, w celu ograniczenia w macierzy pomyłek przypadków fałszywie ujemnych (tzn. błędnego sklasyfikowania pacjenta, który nie przeżyje, jako pacjenta, któremu uda się przeżyć chorobę) zdefiniowano macierz kosztów pomyłek o następującej postaci:

```{r costMatrix, results='markup'}
costMatrix %>% kbl() %>% kable_classic_2(full_width = F) %>% row_spec(0, bold = T) %>% column_spec(1, bold = T)
```

Tak zdefiniowana macierz oznacza, że koszt sklasyfikowania przyszłego denata jako ozdrowieńca jest 15 razy większy, niż odwrotna klasyfikacja. Powinno to pomóc w maksymalizacji wskaźnika czułości (ang. *sensitivity*)

Macierz pomyłek dla zbioru testowego dla zbudowanego klasyfikatora przedstawia się w następujący sposób:

```{r confMatrix, results='markup'}
dimnames(confMatrix$table)$Prediction <- c('przewidywany zgon', 'przewidywane przeżycie')
dimnames(confMatrix$table)$Reference <- c('zgon', 'przeżycie')
confMatrix$table %>% kbl() %>% kable_classic_2(full_width = F) %>% row_spec(0, bold = T) %>% column_spec(1, bold = T)
```

Podsumowanie podstawowych miar klasyfikacji:

```{r confMatrixSummary, results='markup'}
confMatrix$byClass %>% kbl() %>% kable_classic_2(full_width = F) %>% column_spec(1, bold = T)
```

Jak widać, czułość dla tego klasyfikatora na zbiorze testowym wynosi 100%.