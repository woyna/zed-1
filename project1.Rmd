...
```{r gender_summary}
gender_summary <- blood_test_grouped %>% mutate(gender = if_else(condition = gender == 1, 'female', 'male')) %>% mutate(survived = if_else(condition = outcome == 0, 'yes', 'no')) %>% group_by(gender, survived) %>% count(name = 'count') %>% arrange(desc(gender))
```

```{r gender_summary_table, results='markup'}
gender_summary %>% kbl() %>% kable_classic_2(full_width = F) %>% row_spec(0, bold = T)
```

```{r gender_age_summary_plot, results='markup'}
gender_age_survived_summary <- blood_test_grouped %>% mutate(age_group = factor(1*(age > 35) + 1*(age > 55), labels = c('18-35', '36-55', '56+'))) %>% mutate(gender = if_else(condition = gender == 1, 'female', 'male')) %>% mutate(survived = if_else(condition = outcome == 0, 'yes', 'no')) %>% group_by(gender, age_group, survived) %>% count(name = 'count')

gender_age_summary_plot <- ggplot(gender_age_survived_summary, aes(x = gender, y = count, fill = survived)) + geom_col(width = 0.4) + facet_grid(. ~ age_group) + theme_light()
...
```{r neutrophils}
neutrophils <- blood_test_grouped %>% select(neutrophils..., neutrophils.count, outcome) %>% mutate(outcome = if_else(condition = outcome == 0, 'yes', 'no')) %>% rename(survived = outcome) %>% mutate(survived = factor(survived))
```

```{r neutrophils_plot, results='markup'}
neutrophils_plot <- ggplot(neutrophils, aes(x=neutrophils.count, y=neutrophils..., color=survived)) + geom_point() + geom_hline(yintercept=range(percent_min, percent_max), linetype='dashed') + geom_vline(xintercept=range(count_min, count_max), linetype='dashed') + labs(x = 'neutrophils [%]', y='neutrophils [thousand cells/µl]') + theme_light()
...
```{r ldh}
ldh <- blood_test_grouped %>% select(Lactate.dehydrogenase, outcome, age) %>% mutate(outcome = if_else(condition = outcome == 0, 'yes', 'no')) %>% rename(survived = outcome, LDH = Lactate.dehydrogenase) %>% mutate(survived = factor(survived))
```

```{r ldh_plot, results='markup'}
ldh_plot <- ggplot(ldh, aes(x=age, y=LDH, color=survived)) + geom_point() + geom_hline(yintercept=range(ldh_min, ldh_max), linetype='dashed') + labs(y='LDH [U/l]') + theme_light()
ggplotly(ldh_plot)
```
